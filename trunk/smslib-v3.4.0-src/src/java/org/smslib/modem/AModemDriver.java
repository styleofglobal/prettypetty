...
public abstract class AModemDriver
{
		...
		public void run()
		{
			while (isConnected())
			{
				try
				{
					...
					if (getGateway().getService().getInboundNotification() != null)
					{
	...
	private class CNMIEmulatorProcessor extends ASchedulerTask
	{
		private List<InboundMessage> msgList;

		public CNMIEmulatorProcessor(String myName, Service myService)
		{
			super(myName, myService);
		}
		...
		public void process()
		{
			if ((isConnected()) && (getGateway().getStatus() == GatewayStatuses.STARTED) && (getGateway().getService().getInboundNotification() != null))
			{
				try
				{
					synchronized (getSYNCInboundReader())
